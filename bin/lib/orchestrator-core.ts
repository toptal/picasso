...
function buildStuckRecoveryHappoPrompt(
  ...
  return (
    ...
    `### 1b. \`dimension_mismatch\`, or the box measures the same but Happo still differs → diff OLD source vs NEW\n\n` +
    `A size change is ALWAYS a dropped/changed CSS property in YOUR diff, never environmental. Compare the **pre-migration** styles — the old \`createStyles\` AND any \`PicassoProvider.override(() => ({ Mui<Name>: ... }))\` for this component — against your new Tailwind classes, property by property. The classic miss is a pinned **\`line-height\`**: MUI pinned it; \`text-[…]\` sets font-size only, so dropping it leaves \`line-height: normal\` (≈1.07× on proxima-nova) → ~1px reflow on every story (Checkbox PF-1994). Restore it as \`leading-*\`. Also check \`letter-spacing\` / \`vertical-align\` / \`box-sizing\`.\n\n` +
    `**Font caveat:** a font-metric diff will NOT reproduce on localhost — proxima-nova is domain-locked and falls back to Arial there (\`references/visual-verification.md §"The production font is domain-locked"\`). "Can't reproduce locally" is the EXPECTED fallback, not proof the diff is environmental/unfixable. Happo's cloud render is authoritative; diff the source, not the local boxes.\n\n` +
    ...
    `- Don't call the diff "environmental" / "a font thing" / "unfixable" because it didn't reproduce on localhost (proxima-nova falls back to Arial there) or because the box measures the same — a \`dimension_mismatch\` is always a real property change in your diff. Do the step-1b OLD-vs-NEW source diff first.\n\n` +
...
function buildHappoFailureSection(
  ...
  return (
    ...
    '   - `verdict: dimension_mismatch` — width or height differs between baseline and after. The element changed SIZE, not position. Look at `box-sizing`, `padding`, `border-width`, `line-height`, `width`/`height` setters, or content reflow. This is ALWAYS a real property change in your diff — never environmental, never an approved-delta. Diff the pre-migration source (old `createStyles` + `PicassoProvider.override`) against your Tailwind; a dropped pinned `line-height` (becomes `line-height: normal`) is the classic miss (Checkbox PF-1994). A font-metric diff will not reproduce on localhost (proxima-nova → Arial fallback) — not grounds to call it unfixable.\n' +
