...
- **If a pnpm patch must ship**, co-locate it under `packages/<pkg>/patches/` and reference it from **`pnpm-workspace.yaml` `patchedDependencies`** (pnpm 10 location — not `package.json`) — never gitignore it at repo root (consumer-package CI installs fail otherwise; Modal iters 1/5). The TS 4.7 `@base-ui/utils@0.2.8` `const`-stripping patch was **removed** after the TS 5.5 upgrade (TS 5.x parses `const` type params natively) — do not re-add it.
