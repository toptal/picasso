...
direct `[data-picasso-popper]` selection, `check`/`uncheck({ force: true })`,
and `trigger('mouseover'|'mouseenter', { force: true })`:
...
| A same-day date range cannot address the day twice by testid, or `day-button-<n>` matches two cells                                                                                           | `CalendarDay`'s testid **mutates with selection state** (`day-button-<n>` → `day-button-selected`), and outside-month cells stamp the same number as in-month ones. Both are upstream smells — but the calendar already ships a **stable, unique** hook: `data-calendar-day="<ISO date>"`. Select on that (`cy.getPopup().find('[data-calendar-day="2026-08-31"]')`) instead of the testid, and neither problem applies.                                                                                   |
