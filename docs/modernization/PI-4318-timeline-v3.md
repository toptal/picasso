...
**Visual legend (color = track, ★ prefix = critical path):**

| Visual                                           | Meaning                                                                               |
| ------------------------------------------------ | ------------------------------------------------------------------------------------- |
| 🟦 **Blue bar** (default)                        | **Modernization track** ticket (PF-1988 epic) — regardless of which engineer executes |
| 🟩 **Green bar** (`active` tag)                  | **Agent Experience track** ticket (PF-1989 epic)                                      |
| 🟧 **Orange bar** (`done` tag)                   | **Figma Design-to-Code track** ticket (PF-1990 epic)                                  |
| 🟪 **Purple bar** (`crit` tag, theme-overridden) | **Maestro Integration track** ticket (PF-1991 epic)                                   |
| **★** prefix in label                            | **Critical path** — slipping this task slips program end                              |
| (no ★)                                           | Non-critical — has slack                                                              |

Color is determined by the ticket's parent epic (track), not by which engineer's row it sits in. So PF-2009 (Code Connect 55) is **orange** even when it appears on Eng A's row, because Code Connect is Figma-track work.
...
config:
  ...
  themeVariables:
    ...
    sectionBkgColor2: '#F4F6F7'
    taskBkgColor: '#3498DB'
    taskBorderColor: '#1F618D'
    taskTextColor: '#FFFFFF'
    taskTextDarkColor: '#1B2631'
    taskTextLightColor: '#FFFFFF'
    taskTextOutsideColor: '#1B2631'
    activeTaskBkgColor: '#27AE60'
    activeTaskBorderColor: '#196F3D'
    doneTaskBkgColor: '#E67E22'
    doneTaskBorderColor: '#A04000'
    critBkgColor: '#8E44AD'
    critBorderColor: '#5B2C6F'
    gridColor: '#D5D8DC'
  gantt:
    leftPadding: 120
    rightPadding: 30
...
---
gantt
    title PI-4318 Timeline v3 — colored by track
...
| Tag      | Theme variable                                                                  | Track                              |
| -------- | ------------------------------------------------------------------------------- | ---------------------------------- |
| (no tag) | `taskBkgColor` = blue (`#3498DB`)                                               | **Modernization** (PF-1988)        |
| `active` | `activeTaskBkgColor` = green (`#27AE60`)                                        | **Agent Experience** (PF-1989)     |
| `done`   | `doneTaskBkgColor` = orange (`#E67E22`)                                         | **Figma Design-to-Code** (PF-1990) |
| `crit`   | `critBkgColor` = purple (`#8E44AD`) — theme-overridden, **not** the default red | **Maestro Integration** (PF-1991)  |

**How critical path is shown:**

The `crit` tag is repurposed for the Maestro track in this Gantt (Mermaid only has 4 base tag styles, so all four had to be assigned to tracks). To indicate critical path, **task labels are prefixed with ★**. Critical path tickets in v3:

- ★ PF-1992 Migration plan
- ★ PF-1993 pnpm migration
- ★ PF-1994 + PF-2024 + PF-2025 (the base/\* tier chain)
- ★ PF-2021 query-builder
- ★ PF-2023 picasso-provider canary
- ★ PF-2009 Code Connect 55 swarm (both bars — on Eng A and Eng C)

> **Mermaid limitation worth knowing:** Mermaid Gantt has 4 base tag styles (`default / active / done / crit`) and they don't compose. We've used all four for the four tracks, so critical path can't get its own bar style. The `★` prefix in the label is the visual cue. Section row backgrounds are subtle (light grey alternating) to keep focus on the track-colored bars rather than competing for attention.
...
- Skim **vertically** by row to see what each engineer is working on across phases
- Skim **horizontally** by color to track each track's progression — Mod (blue), AIC (green), Figma (orange), Maestro (purple)
- Look for **★** to spot what's on the critical path
