...
import { breakpointsList } from '@toptal/picasso-provider'
...
const generateClassesForNonXsBreakpoints = (breakpointKey: string, breakpointValue?: GridSize | boolean) => {
  ...
  const breakpointValueClassName = breakpointValue === 12 ? 'full' : `${breakpointValue}/12`

  return [`${breakpointKey}:basis-${breakpointValueClassName} ${breakpointKey}:max-w-${breakpointValueClassName}`]
}
...
export const GridItem = forwardRef<HTMLDivElement, Props>(function GridItem(
  ...
  const breakpointAndColumnClassName = getBreakpointAndColumnClassName({xs, sm, md, lg, xl}).join(' ')


  return (<div
    className={`${spacingClassName} ${breakpointAndColumnClassName} `}
    style={style}
  >
    {children}
  </div>)
