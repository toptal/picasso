...
- **Global reset & box model** — the page-level reset ships as `@toptal/picasso-tailwind/base` (imported in the consumer's Tailwind entry CSS, emitted in `@layer base`): content-box page box model (the legacy CssBaseline carve-out — Picasso components stay border-box via the PicassoRootNode island), body/root layout, font smoothing. It's a baseline — utilities and unlayered app CSS always win over it. Tailwind's full preflight stays off; opting out of the reset = omitting the import.
...
- **Build before you snapshot.** Run `build:package` before `jest -u`; when you drop a workspace dep from `package.json`, also drop its `references` entry from `tsconfig.json`. Take `versionBump` verbatim from `docs/migration/manifest.json`. JSS→Tailwind mechanics live in `docs/migration/rules/jss-to-tailwind-crib.md`.
