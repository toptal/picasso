...
- **Global reset & box model** — the page-level reset ships as `@toptal/picasso-tailwind/base` (imported in the consumer's Tailwind entry CSS, emitted in `@layer base`): border-box page box model, body/root layout, font smoothing. It's a baseline — utilities and unlayered app CSS always win over it. Tailwind's full preflight stays off; opting out of the reset = omitting the import.
