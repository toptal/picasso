 ...
 */
function getNextWrappingIndex(moveAmount, initialIndex, itemCount) {
...
const useAutocomplete = ({ value, options = [], enableAutofill = false, autoComplete, onChange = () => { }, onKeyDown = () => { }, onSelect = () => { }, getDisplayValue }) => {
    ...
    const getInputProps = () => ({
        ...
        onKeyDown: (event) => {
            ...
            if (key === 'ArrowUp') {
                event.preventDefault();
                setOpen(true);
                setHighlightedIndex(getNextWrappingIndex(-1, highlightedIndex, options.length));
            }
            if (key === 'ArrowDown') {
                event.preventDefault();
                setOpen(true);
