...
const FileInputContent = withStyles(styles)(({ classes, accept, onChange, value, status, disabled, error, progress, inputRef }) => {
    const getFilename = () => {
        if (error || progress) {
            return status;
        }
    ...
    return (React.createElement(Fragment, null,
        React.createElement(Typography, { className: cx(classes.inputValue, {
                [classes.inputValueDisabled]: disabled
            }), inline: true }, getFilename()),
        React.createElement("input", { type: 'file', className: classes.nativeInput, ref: inputRef, accept: accept, onChange: onChange })));
});
export const FileInput = forwardRef(function FileInput({ classes, className, style, width, accept, progress, error, disabled, value, status, onChange }, ref) {
    ...
    const inProgress = (isNumber(progress) && progress <= 100) ||
