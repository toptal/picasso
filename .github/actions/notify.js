...
const baseUrl = new URL(`https://github.com/toptal/picasso/blob/${GITHUB_REF}/`)
...
async function generatePackageMap() {
  ...
  await Promise.all(
    packages.map(async pkg => {
      ...
      const content = await readFile(pkgFile).then(res =>
        JSON.parse(res.toString())
      )
      packageMap.set(
        content.name,
        `./packages/${pkg.replace('/package.json', '')}`
      )
    })
  )
}
...
async function run() {
  ...
  const packages = await Promise.all(
    data.map(({ name, version }) => {
      ...
      return {
        ...
        version,
        url: new URL(
          `${p}/CHANGELOG.md#${version.replace(/\./g, '')}`,
          baseUrl
        ).toString(),
      }
    })
  ...
  if (message.length < 2000) {
    ...
    const { name, version, url } =
      packages.find(pkg => pkg.name === 'astro') ?? packages[0]
    message = `${emoji} Some ${descriptor} ${pluralize(verb)}\n\n`
    message += `• \`${name}@${version}\` Read the [release notes →](<${url}>)\n`
