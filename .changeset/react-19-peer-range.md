...
- consumer notes from validating this release in Staff Portal on React 18 and then on React 19: `@topkit/modals-service@4.0.0` declares exact-version peers on ten Picasso packages, so once a workspace moves to any newer Picasso, pnpm's `auto-install-peers` nests a second Picasso tree under it (27 packages with their own `@toptal/picasso-shared` and `notistack`) and modals render in a separate Picasso and notistack instance — until modals-service publishes range peers, pin its peers to the workspace versions with scoped overrides (`'@topkit/modals-service>@toptal/picasso-*'`) or bump it in lockstep with Picasso
- `logrocket-react@6.0.3` (through `@topkit/analytics`) does not support React 19: it warns `"logrocket-react does not work with this version of React"` at module load and disables its React integration, which `jest-fail-on-console` turns into a failure in every suite that imports it — allowlist the warning or drop the plugin until a release supports React 19
