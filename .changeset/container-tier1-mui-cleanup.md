...
- Drop vestigial `classes` prop from the public type via `Omit<StandardProps, 'classes'>`. The prop was inherited from `StandardProps` but was never read in source and had zero internal / external consumer usage per the cross-tier audit; a runtime destructure backstop strips any legacy passthrough so it can't reach the DOM.
