...
- add `@toptal/picasso-cypress-utils` — Cypress commands written against Picasso's DOM contract, so consumer specs state intent instead of walking the DOM. Register them from a Cypress support file with `registerPicassoCypressCommands()`, which takes a `skip` option for repos that already own one of the names. The major is the version jump, not a break: `0.0.0` on npm is a placeholder from bootstrapping the package, and this first real release joins the v100 line
...
- `createCypressOverride()` now returns `excludedFiles: ['**/cypress/support/**']` and takes an `excludedFiles` argument. The default globs sweep `**/cypress/**`, which includes the support layer — the one place that legitimately touches the raw markers the bans point away from, since it is where the commands are registered. Anyone using the ready-made helper verbatim was flagging their own command layer
- add a shareable `no-restricted-syntax` config at `@toptal/picasso-cypress-utils/eslint` that bans selecting `[data-picasso-popper]` directly, `check`/`uncheck({ force: true })`, `trigger('mouseover'|'mouseenter', { force: true })`, and selecting `[role="tooltip"]` directly
