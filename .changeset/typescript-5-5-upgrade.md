...
- the `OverridableComponent<P>` type in `@toptal/picasso-shared` gains an additional ref-attribute call signature (`<R = unknown>(props: P & RefAttributes<R>) => JSX.Element | null`) listed AFTER the existing polymorphic `as`-prop signature. The new overload exists so `forwardRef<R, P>(...)` assigns cleanly under TS 5.x's stricter assignability when `P` contains required fields; the declaration order means plain `<X />` and `<X as={...} />` callers continue to resolve to the original signature and keep their pre-5.5 inference behavior.
